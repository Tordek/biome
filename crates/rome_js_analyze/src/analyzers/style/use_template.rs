...
impl Rule for UseTemplate {
    ...
    fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
        ...
        let template = template_expression_from(node)?;
...
/// This is the case, if:
///
/// - the operation has `+` as operator,
/// - the operation contains a string-like literal and a non-string-like
///
...
fn template_expression_from(node: &JsBinaryExpression) -> Option<JsTemplateExpression> {
    // While `template_elements` is empty, we keep track of last leaved node.
    // Once we see the first string/template literal,
    // we insert `last_leaved_node` in `template_elements` and the seen string/template literal.
    // ANy succeeding expression is directly inserted in `template_elements`.
